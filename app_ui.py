...
with st.sidebar:
    ...
    st.info("Generate a free development key at Google AI Studio instantly.")

if not api_key:
    st.warning("Please provide a valid Google AI API key in the sidebar panel to connect the infrastructure pipeline.")
    st.stop()

# Initialize core sessions cleanly
if "water_client" not in st.session_state:
    st.session_state.water_client = engine.initialize_water_bot(api_key)
...
if user_input := st.chat_input("Ex: 'A main pipe split open on 5th Avenue and it's flooding the street'"):
    ...
    with st.chat_message("assistant"):
        with st.spinner("AI analyzing infrastructure report telemetry..."):
            # FIX: Route conversational states safely across the modernized SDK function structures
            raw_response = engine.send_ai_message(
                st.session_state.water_client, 
                user_input, 
                st.session_state.chat_display_logs[:-1]
            )
